...
class ExportWrapper(torch.nn.Module):
    ...
    def __init__(self, model: torch.nn.Module, value_scale: float) -> None:
        super().__init__()
        self.model = model
    ...
    def forward(self, x: torch.Tensor):
        ...
        value_points = value_scaled * self.value_scale          # raw points
...
def export_one(ckpt: Path, out: Path, net: str,
               value_scale: float | None, opset: int) -> None:
    ...
    wrapper = ExportWrapper(model, cfg.value_scale).eval()
...
def main() -> int:
    ...
    export_one(args.ckpt, args.out, args.net, args.value_scale, args.opset)
    return 0
