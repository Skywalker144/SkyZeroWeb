...
- **ponder（对弈模式你的回合 / 分析模式任意手）**：固定 `ANALYSIS_CHUNK = 128` sims/块的 PUCT（对齐 V7.19 play.cfg），结果回来后只要**累计根访问 < `ANALYSIS_CAP_MIN = 2000`** 就复用树再下一块，直到 ≥2000 才停（见结果处理 `main.js`）。对弈模式下安静跑（状态栏只显示「轮到你」），但候选/胜率/热力图实时刷新；你一落子就 `searchId++` 中止当前块。
- **move-search（对弈模式 AI 自己的回合）**：anytime PUCT，跑满 `thinkMs`（工具栏「思考时间」下拉，默认 1000ms）**或**累计根访问到 `SEARCH_VISIT_CAP`（`worker.js`，现 = 2000，与 ponder 上限对齐）——**先到者停**，再按 V7.19 的 retrospective weight + LCB 选着。沿用 KataGo threshold `0.95`，并把 `searchFactorWhenWinning` 设为 `0.30`：最近 3 次完成的 AI 落子搜索都极优时，把时间和访问上限从 100% 平滑降到最低 30%；ponder 结果不计入这 3 次。`thinkMs` 不会加深你的回合 ponder（后者永远是 128-块到 2000 封顶）。
...
- 浏览器版五子棋允许用户在 `-1..1` 调节 PDA（步长 `0.1`、默认 `0.5`，中点附近吸附到平衡 `0`）：对弈模式参考方始终是 AI 颜色，分析模式固定参考黑方；PDA 只改变 NN 条件、不改变搜索量，数值或参考方变化都必须清树和 NN cache。相对 C++ play 的必要适配只有单线程/单 leaf 和每次 cache miss 单个随机 D4；RNNM 明确排除。细节见 `README.md` 的「V7.19 browser search contract」。
