...
- **move-search（对弈模式 AI 自己的回合）**：anytime PUCT，跑满 `thinkMs`（工具栏「思考时间」下拉，默认 3000ms）**或**累计根访问到 `SEARCH_VISIT_CAP`（`worker.js`，现 = 2000，与 ponder 上限对齐）——**先到者停**，再按 V7.19 的 retrospective weight + LCB 选着。`thinkMs` 只影响 AI 自己这一手的搜索时长，不会加深你的回合 ponder（后者永远是 128-块到 2000 封顶，与 `thinkMs` 无关）。
...
- 浏览器版五子棋相对 C++ play 的必要适配只有单线程/单 leaf 和每次 cache miss 单个随机 D4；RNNM 明确排除。细节见 `README.md` 的「V7.19 browser search contract」。
