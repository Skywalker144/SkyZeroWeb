...
- 浏览器版五子棋固定使用 `PDA=0.5`：对弈模式参考方始终是 AI 颜色，分析模式固定参考黑方；PDA 进入 NN cache key，参考方变化必须清树和缓存。相对 C++ play 的必要适配只有单线程/单 leaf 和每次 cache miss 单个随机 D4；RNNM 明确排除。细节见 `README.md` 的「V7.19 browser search contract」。
